 ...
 * Time: 16:09
 */
class CreateAgencyAfterBox  extends BaseJob
{
    /**
     * get the queue name
    ...
    protected function realHandler()
    {
        ...
        checkLogic(!empty($agencyInfo), '找不到出库单为'.$sm_order.'的省代售后信息');
        ...
        foreach ($agencyInfo as $key => $item) {
            ...
            $checkBox = DB::table('t_order_box')->where('box_number', $item['box_number'])->count();
            ...
            if ($checkLineInfo <= 0) {
                $result = curl_post_omsapi('aftersales/aftersales_info', ['aftersales_no' => $item['order_number']]);
                checkLogic($result['code'] == 200, '从OMS获取线路信息失败1'.$result['msg']);
                checkLogic(!empty($result['data']), '从OMS获取线路信息失败2'.$result['msg']);
                //api日志
                ApiRequestLog::saveLog(
                    1, 
                    'SaveAgencyConsignOut',
                    env('OMS_API_URL','').'aftersales/aftersales_info',
                    json_encode(['aftersales_info'  => $item['order_number']]),
                    $result['msg']
                );
                // 箱子的线路线路信息
