...
class XLSXWriter implements XLSXWriterContract
{
	...
	protected $column_type_set = []; // 自定义设置
     ...
     * @param type $filename
     */
    public function export($data, $filename, $column_type = []) {
        $max = 100000;
        if (count($data) > $max) {
            exit("导出数据超出{$max}条");
        }
        foreach ($column_type as $ctk => $ctv) {
            $key = array_search($ctk,$data[0]);
            if (false !== $key) {
                $this->column_type_set[$key] = $ctv;
            }
        }
        $filename .= '.xlsx';
        ...
        $this->writeSheet($data);
        $this->writeToStdOut($filename);
        exit;
    }
     ...
     */
    public function exportToFile($data, $filename, $column_type = []) {
        foreach ($column_type as $ctk => $ctv) {
            $key = array_search($ctk,$data[0]);
            if (false !== $key) {
                $this->column_type_set[$key] = $ctv;
	...
	public function writeSheetRow($sheet_name, array $row, $row_options=null)
	{
		...
		if (count($sheet->columns) < count($row)) {
                        $arr = array_fill($from=0, $until=count($row), 'GENERAL');
                        foreach ($this->column_type_set as $k => $v) {
                            $arr[$k] = $v;
                        }
			$default_column_types = $this->initializeColumnTypes($arr);
//			$default_column_types = $this->initializeColumnTypes( array_fill($from=0, $until=count($row), 'GENERAL') );// 默认值
