...
class MyRedis
{
    ...
    /**
     * 在队列尾部插入一个元素 如果key不存在，什么也不做.返回队列长度
     * @param $key
     * @param $value
     * @return int
     */
    ...
    /**
     * 在队列头插入一个元素 如果key不存在，什么也不做.返回队列长度
     * @param $key
     * @param $value
     * @return int
     */
    ...
    public function createKey(string $format, array $value = NULL)
    {
        if (is_null($this->flag)) {
            $this->flag = env('REDIS_FLAG');
        }
        if (!empty($value)) {
            $key = vsprintf($format, $value);
        } else {
            $key = $format;
        }
