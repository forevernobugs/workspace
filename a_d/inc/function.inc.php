...
*分页
**/
function getpage($table,$choose,$url,$rows=20,$total='',$parapidan=null){
	global $page,$db;
	if(!$page){
		$page=1;
	}
    ...
    for($j=$s;$j<=$b;$j++){
        if($j==$page){
            ...
            $show.=" <a href=\"$url&page=$j$parapidan\" title=\"第{$j}页\">$j</a>";
        }
    }
    $showpage="<a href=\"$url&page=1$parapidan\" title=\"首页\">首页</A> <a href=\"$url&page=$uppage$parapidan\ title=\"上一页\">上一页</A>  {$show}  <a href=\"$url&page=$nextpage$parapidan\" title=\"下一页\">下一页</A> <a href=\"$url&page=$totalpage$parapidan\" title=\"尾页\">尾页</A> <a href='#' class=zs>$page/$totalpage</a>";
    if($totalpage>1){
        return $showpage;
    }
}
