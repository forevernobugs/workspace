...
isset($propagandize_uid) && $propagandize_uid = intval($propagandize_uid);
check_parameter();
function check_parameter(){
	$array_key = array('typeid','iframeID','job','jobs','ad_id','styletype','type','psid','TagId','u','showDomain');
...
if($lfjdb[yz]){
	...
	if($webdb[loginGive] && $webdb[loginTime] && $timestamp-$lfjdb[lastvist]>$webdb[loginTime]*3600 ){
		add_user($lfjdb['uid'],$webdb[loginMoney],'回头客，定期回访奖励！');
		$usr_oltime=0;	//强制重新写入lastvist
	}
