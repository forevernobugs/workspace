...
function rollpic($rolldb){
	...
	$show.="<li><a href='$piclink[$i]' target='$link_to_ck'><img src=\"$picurl[$i]\" thumb=\"$picurl[$i]\" alt=\"$picalt[$i]\" text=\"$pictxt[$i]\" width=\"$width\" height=\"$height\"/></a></li>";
	}
	$show="
	<div id='myFocus-wrap'>
    <div id='$rand' style='visibility:hidden'>
...
function getParam(){//获取全部参数
	return {
		path:'$webdb[www_url]/images/default/rollpic/',
		id:'$rand',
		pattern:'$rolltype',
		time:'$times',
		trigger:'$mo_ck',
...
function CMS_sp_article($format,$rsdb,$Rows){
	...
	if(strstr($format[tplpart_1code],'$content')){
		$SQL="SELECT A.*,B.content,A.aid AS id FROM {$_pre}article A LEFT JOIN {$_pre}reply B ON A.aid=B.aid WHERE A.aid IN ($rsdb[aids]) AND B.topic=1 $_SQL ORDER BY $order DESC LIMIT $Rows";
	}else{
		$SQL="SELECT A.*,A.aid AS id FROM {$_pre}article A WHERE A.aid IN ($rsdb[aids]) $_SQL ORDER BY $order DESC LIMIT $Rows";
	}
	$query = $db->query($SQL);
	while($rs = $db->fetch_array($query)){
