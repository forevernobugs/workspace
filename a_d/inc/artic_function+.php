...
function query_article_module($mid,$type,$post_db,$basedb){
	...
	foreach( $post_db AS $key=>$value)
	{
		if(is_array($value))
		{
			$post_db[$key]=filtrate(implode("/#/",$value));
			$post_db[$key] && $post_db[$key]="/#/".$post_db[$key]."/#/";
		}
...
function show_module_content($m_config){
	...
	foreach( $m_config[field_db] AS $key=>$rs )
	{
		...
		{
			if($kv=strstr($rs['form_set'],"{$rsdb[$key]}|")){
				$detail2=explode("\r\n",$kv);
				$rsdb[$key]=substr($detail2[0],strlen("$value|"));
			}
		}
		elseif($rs[form_type]=='checkbox')
		{
...
function listpage_title($fid,$type,$rows,$leng,$id,$keyword){
	...
	$listdb = fetch_article($fid,$type,$rows,$leng,$id,$keyword);
...
//获取最热,最新,推荐,及相关文章
function fetch_article($fid,$type,$rows,$leng,$id='',$keyword=''){
