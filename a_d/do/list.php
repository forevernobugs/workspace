...
$listdb=ListThisSort($rows,$webdb[ListLeng]?$webdb[ListLeng]:50,$select_wheresql);		//本栏目文章列表
$page_sql=$webdb[viewNoPassArticle]?'':' AND yz=1 ';
$erp=$fidDB[iftable]?$fidDB[iftable]:"";
$showpage=getpage("{$pre}article$erp","WHERE fid=$fid $page_sql $select_wheresql2","list.php?fid=$fid",$rows,null,strstr($selectlink,"&"));	//文章列表分页
