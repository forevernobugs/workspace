...
if($job=="list"&&$Apower[member_list])
{
	...
	while($rs=$db->fetch_array($query)){
		...
		if($rs[yz]){
			$rs[yz]="<A HREF='index.php?lfj=$lfj&action=yz&uid_db[0]=$rs[uid]&T=noyz' style='color:red;' onclick=\"$rs[alert]\" title='已经通过审核,点击即可取消审核'><img src='../images/member/check_yes.gif' border='0'></A>";
		}elseif($rs[groupid]){
			$rs[yz]="<A HREF='index.php?lfj=$lfj&action=yz&uid_db[0]=$rs[uid]&T=yz' style='color:blue;' onclick=\"$rs[alert]\" title='还没有通过审核,点击即可通过审核'><img src='../images/member/check_no.gif' border='0'></A>";
		}else{
			$rs[yz]="未激活";
		}
